...
bool segments_intersection(double x_a, double y_a, double x_b, double y_b, double x_c, double y_c, double x_d, double y_d)
{
    ...
    //ETAPE 0 : g�nerer les deux droites (AB) et (CD) � partir des points sous la forme ax + by + c = 0
    ...
    else {//Sinon la droite (AB) est oblique. On met b = 1 et on calcule a et c pour avoir l'�quation ax + 1y + c = 0 (pour �viter les divisions par 0)
        a = (y_b - y_a) / (x_a - x_b);
        b = 1;
    ...
    else {//Sinon la droite (CD) est oblique. On met b = 1 et on calcule a et c pour avoir l'�quation ax + 1y + c = 0 (pour �viter les divisions par 0)
        a2 = (y_d - y_c) / (x_c - x_d);
        b2 = 1;
        c2 = (y_c * x_d - y_d * x_c) / (x_c - x_d);
    }

    // ETAPE 1 : d�terminer si il y a intersection entre (AB) et (CD)

    double determinant = a * b2 - a2 * b; //pour d�terminer s'il y a intersection il faut calculer le d�terminant

    if (determinant != 0) { //les droites se croisent en un point d'intersection.
        //Le point d'intersection (Px, Py) est calcul� par :
        Px = (b * c2 - b2 * c) / determinant;
        Py = (a2 * c - a * c2) / determinant;
    }
    else { //les droites sont parall�les ou align�es
        if ((a * b2 == a2 * b) && (a * c2 == a2 * c) && (b * c2 == b2 * c)) { //Verifier si les droites sont align�es. J'ai �vit� ((a / a2) == (b / b2) && (b / b2) == (c / c2)) pour ne pas avoir � faire des tests � cause de la division.
    ...
    // ETAPE 2 : D�terminer si le point d'intersection est situ� entre les bornes des deux segments.

    if (((Px >= x_a && Px <= x_b) || (Px >= x_b && Px <= x_a)) //On v�rifie que le point d'intersection (Px, Py) est bien sur les deux segments.
...
int main() {
    ...
    //Test 7 : segments parall�ls : 0 (faux)
